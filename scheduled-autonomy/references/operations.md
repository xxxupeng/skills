...
本版本后端：Linux、`systemd --user`、Python3 和 `websocket-client`。
不自动安装依赖、重启服务或公开网络端口。

入口为本 Skill 目录的 `scripts/scheduler.py`，下文 `S` 指该脚本绝对路径。
使用同一个 Python 解释器（如 `/usr/bin/python3`）执行与部署，确保它能导入 `websocket`。
...
`init` 对现有服务执行 initialize + thread/read，并核对 UUID/cwd，不创建/恢复会话；到点投递前会恢复同一ID。
...
python3 "$S" init --objective '获批目标' --criteria '固定验收标准' \
  ...
  --deadline '2026-10-01T12:00:00+08:00' --min-interval 3600 --native-goal-inactive
python3 "$S" probe
...
未到期时不连接 app-server；到期若会话 active，每10分钟重查一次，最多一小时/目标截止时间。
...
完成状态保存到last_delivery；完成但未安排后继且未明确结束目标则waiting_user并告警。结果不明不重发。
...
`python3 -m unittest discover -s <skill>/tests -v` 使用隔离临时状态与模拟 peer，不发送真实消息。
`probe` 对现有 app-server 只读。真实首次投递建议只要求确认收到，禁止启动实验。
来源：[官方 App Server 协议](https://learn.chatgpt.com/docs/app-server)。
